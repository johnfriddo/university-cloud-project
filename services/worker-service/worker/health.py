...
Il worker consuma da una coda, quindi non gli serve né Flask né gunicorn: un
server della libreria standard su un thread di servizio basta alle sonde del
kubelet, e tiene l'immagine libera da uno stack web che non userebbe mai.
"""
...
def _make_handler(service: str, version: str, readiness: ReadinessFn):
    class Handler(BaseHTTPRequestHandler):
        ...
        def log_message(self, fmt, *args):
            # Le sonde bussano qui ogni pochi secondi: tenerle fuori dal registro.
            log.debug(fmt, *args)
